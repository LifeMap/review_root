...
### 2.2 Problem
- 지그재그는 여성 패션 버티컬의 핵심 플랫폼 중 하나로, PRD v1.4에서 MVP 지원 플랫폼으로 확정됨
- 접근 방식(공개 API 존재 여부, 봇 탐지 강도, 리뷰 데이터 구조)이 아직 조사되지 않아 구현 전 사전 분석이 필요함
- 기존 29cm/무신사와 동일한 4단계 파이프라인 구조에 통합해야 함

### 2.3 Why Now
- PRD v1.4 확정으로 지그재그가 MVP 범위에 포함됨
- 무신사 파이프라인 구조가 안정화되어 새로운 플랫폼 추가를 위한 베이스가 마련됨

## 3. Goals

### 3.1 Primary Goals
1. 지그재그 데이터 파이프라인 구현 (`platforms/zigzag.ts`)
2. 29cm/무신사와 동일한 4단계 구조 적용
3. 지그재그 전용 테스트 스크립트 구현 (`scripts/test-zigzag-crawl.ts`)
4. 환경변수 `MAX_ZIGZAG_PRODUCTS` 지원

### 3.2 Success Criteria
| Metric | Target |
|--------|--------|
| 스크립트 실행 | 에러 없이 완료 |
| 상품 수집 | MAX_ZIGZAG_PRODUCTS 개수 |
...
> **중요**: 지그재그의 실제 접근 방식은 사전 조사(공개 API 유무, DOM 구조, 인증 요구사항, robots.txt 정책) 완료 후 이 섹션을 업데이트해야 합니다.

### 4.3 Environment Variables
```bash
MAX_ZIGZAG_PRODUCTS=5    # 수집할 상품 수 (기본값 TBD — 조사 후 결정)
...
### 5.1 Files to Create
- `web/src/features/crawlers/platforms/zigzag.ts` — 지그재그 크롤러 구현체
- `web/scripts/test-zigzag-crawl.ts` — 파이프라인 테스트 스크립트

### 5.2 Files to Modify
- `web/src/features/crawlers/registry.ts` — 지그재그 크롤러 등록
...
1. 사전 조사: 지그재그 공개 API 여부, 리뷰 페이지 DOM 구조, robots.txt 확인
2. 접근 방식 결정 (REST API / Playwright / 하이브리드)
3. `zigzag.ts` 크롤러 기본 구조 생성
4. Step 1: 베스트 상품 수집 구현
5. Step 2: 리뷰 크롤링 + DB 저장 로직
6. Step 3/4: AI 분석 및 요약 생성 (29cm/무신사와 동일 로직 재사용)
7. `test-zigzag-crawl.ts` 테스트 스크립트 작성
8. `platforms` 테이블 zigzag 레코드 확인 (id=4)
...
|-------|------|--------|
| Plan | 계획 문서 작성 | ✅ 완료 |
| Research | 사전 조사 (API, DOM, robots.txt) | Not Started |
...
- [Tech Architecture](../../docs/architect/review_service_tech_architecture.md) — 4.3 지그재그 섹션
