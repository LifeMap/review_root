...
| 항목 | 내용 |
|------|------|
| 접근 방식 | TBD (조사 필요) — 공개 REST API 존재 여부, DOM 파싱 가능 여부 미확정 |
| 난이도 | TBD (조사 필요) |
| 프록시 필요 | TBD (조사 필요) |
| 체형 정보 수집 방식 | TBD (조사 필요) — 리뷰 내 구조화 여부 및 텍스트 파싱 가능 여부 미확정 |

> **참고**: 지그재그의 정확한 접근 방식(공개 API 존재 여부, 봇 탐지 강도, 리뷰 데이터 구조)은 사전 조사가 완료된 후 확정됩니다. 상세 내용은 `docs/01-plan/features/zigzag-pipeline.plan.md`를 참조하세요.
...
> **참고**: 에이블리의 정확한 접근 방식(공개 API 존재 여부, 봇 탐지 강도, 리뷰 데이터 구조)은 사전 조사가 완료된 후 확정됩니다. 상세 내용은 `docs/01-plan/features/ably-pipeline.plan.md`를 참조하세요.
